...
impl DataService {
    pub fn convert_excel_to_json(input_path: &str) -> AppResult<String> {
        let mut workbook: Xlsx<_> = open_workbook(input_path)
            .map_err(|e: calamine::XlsxError| AppError::DataConversion(e.to_string()))?;
        ...
        if let Some(Ok(range)) = workbook.worksheet_range(sheet_name) {
            ...
            if let Some(headers) = rows.next() {
                let header_names: Vec<String> = headers.iter().map(|h: &DataType| h.to_string()).collect();

                for row in rows {
                    let mut obj = serde_json::Map::new();
                    for (i, cell) in row.iter().enumerate() {
                        let col_name = header_names.get(i).cloned().unwrap_or_else(|| format!("col_{}", i));
                        let val = match cell.clone() {
                            ...
                            DataType::Duration(f) => json!(f),
                            DataType::DateTimeIso(s) => Value::String(s),
                            DataType::DurationIso(s) => Value::String(s),
    ...
    pub fn convert_csv_to_json(input_path: &str) -> AppResult<String> {
        ...
        for record in rdr.records() {
            ...
            for (i, field) in record.iter().enumerate() {
                ...
                let col_name = headers.get(i).unwrap_or(&fallback);
